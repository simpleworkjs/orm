...
class ORM {
  ...
  async load(modelFiles) {
    ...
    const normalize = (item) => {
      if (typeof item === 'string') {
        ...
        if (exported && exported.prototype instanceof BaseModel) return [exported];
  ...
  // Transaction support (Sequelize only)
  async transaction(fn) {
    if (!this.adapters.sequelize) {
      throw new Error('Transactions are only supported with the Sequelize adapter');
    }
    if (typeof fn !== 'function') {
      // Return a transaction object for manual commit/rollback
      return this.adapters.sequelize.sequelize.transaction();
    }
    // Auto-managed transaction with callback
    return this.adapters.sequelize.sequelize.transaction(async (t) => {
