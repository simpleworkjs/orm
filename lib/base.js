...
class BaseModel {
  ...
  // The access rule per action, as the UI/clients see it. Today this reflects
  // the code-declared `static permissions` (resolved through `permissionsFor`,
  // so built-in defaults are filled in). This is deliberately the single seam
  // for making permissions dynamic later: when access rules move into the DB
  // and become editable at runtime, only this method changes — the OPTIONS
  // payload shape (and every client that reads it) stays the same.
  static permissionsSchema() {
    const out = {};
    for (const action of ['read', 'create', 'update', 'delete']) {
      out[action] = this.permissionsFor(action);
    }
    return out;
  }
