...
class RedisAdapter {
  ...
  registerModel(Model) {
    ...
    Object.defineProperty(BackingModel, 'name', { value: Model.name });

    if (typeof this.Table.register === 'function') {
  ...
  // No native count; count the filtered list. Fine for the modest datasets
  // Redis-backed models are meant for.
  ...
  async create(Model, data) {
    let row;
    if (typeof Model.backingModel.create === 'function') {
      row = await Model.backingModel.create(data);
    } else {
      row = await Model.backingModel.add(data);
    }
